...
bool Reader::SkipToInitialBlock() {
    // Find start of the current block 
...
bool Reader::ReaderRecord(Slice* record, std::string* scratch) {
    ...
    scratch->clear();
    record->clear();
    // Indicates whether data is currently being concatenated across blocks
    bool in_fragmented_record = false;  

    // Record offset of the currently logical record 
    ...
    while (true) {
        ...
        switch (record_type) {
            ...
            default: {
                char buf[40];
                std::snprintf(buf, sizeof(buf), "unknow record type %u", record_type);
...
unsigned int Reader::ReadPhysicalRecord(Slice* result) {
    while (true) {
        ...
        if (kHeaderSize + length > buffer_.size()) {
            ...
            // The end of file has been reached without reading length of payload
