...
// Based on the index_value(contain the offset and the size of data block)
// The corresponding data block is loaded into memory
// Return the capable of traversing the contents of that block of iterator 
...
uint64_t Table::ApproximateOffsetOf(const Slice& key) const {
    ...
    if (index_iter->Valid()) {
        ...
        if (s.ok()) {
            ...
            // We can't decode the block handle in the index block
            // Return the offset of the metaindex block which is close to the whole file size for this case
