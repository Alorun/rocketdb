...
class HandleTable {
    ...
    private:
        ...
        void Resize() {
            ...
            for (uint32_t i = 0; i < length_; i++) {
                ...
                while (h != nullptr) {
                    ...
                    // hash & (new_length - 1) == hash % new_length
        ...
        uint32_t length_;       // The capacity of the hash table array (2^n)
        uint32_t elems_;        // The number of elements
        LRUHandle** list_;      // The hash table list
};
...
class LRUCache {
    public:
        ...
        void Relesse(Cache::Handle* handle);
    ...
    private:
        ...
        // Initialized capacity before use
        size_t capacity_;
        ...
        // Head node of a cold chain list
        // refs==1 and in_cache==true
        LRUHandle lru_ GUARDED_BY(mutex_);

        // Head node of a hot chain list
...
void LRUCache::Relesse(Cache::Handle* handle) {
    MutexLock l(&mutex_);
...
class SharedLRUCache : public Cache {
    public:
        ...
        void Release(Handle* handle) override {
            ...
            shard_[Shard(h->hash)].Relesse(handle);
        }
