...
// First add length, then add the value
...
// The 32 bits are split into four bytes and store in the buffer
// Dst is a pointer to a contiguous block of memory address, 
// this is converted to uint8_t and then used as an array to point to each bytes
// within that contiguous memory adderss
...
// Reassemble the four bytes one by one
